...
class Drawer {
    ...
    constructor(buffer: AudioBuffer, parent: HTMLElement) {
        ...
        this.analyser = this.createAnalyserNode();
    ...
    private updateWaveform() {
        ...
        // Update the graph with the new audio data
        // ...

        // Update the waveform with the audio data
        const audioData = Array.from(this.dataArray).map((value) => value / 128 - 1);
        const node = this.generateWaveform(audioData, {});
        // @ts-ignore
        this.parent.replaceChild(node.node(), this.parent.firstChild);
    }
